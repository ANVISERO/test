...
<form action="/business/register/" method="post">
    ...
    <tr>
      <td></td>
      <td align="left"><input type="submit" value="Сохранить" class="submit" /></td>
    </tr>
   </table>
</form>
...
$users_surveys_q=mysqli_query($link,"select survey_id FROM for_users_corporat_surveys WHERE user_id='".$user_id."'");
...
$surveys_types_q=mysqli_query($link,"SELECT name,id FROM for_survey_types WHERE id in(
    SELECT survey_type_id from for_surveys WHERE id in(".$users_surveys_ids_string.")) order by name");
while ($surveys_types = mysqli_fetch_object($surveys_types_q)) {
    echo "<p><b>".$surveys_types->name."</b></p>";
    echo "<ol>";
    $surveys_q=mysqli_query($link,"SELECT id,name FROM for_surveys WHERE id in(".$users_surveys_ids_string.")
        AND survey_type_id='".$surveys_types->id."'");
