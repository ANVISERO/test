...
$companies_q=mysqli_query($link,"SELECT id FROM base_companies WHERE ".$q_turnover." ".$q_personal." ".$q_city." ".$q_sphere);
...
// Люди в компаниях
$q_people=mysqli_query($link,"SELECT * FROM base_people
where job_id='$job_id'
AND company_id in(".$companies_string.")");
...
$size='700x250';
$img_src_salary_distribution='http://chart.apis.google.com/chart?chof=gif&cht=p&chco=0000ff&chd=t:'.$chd_salary.'&chs='.$size.'&chl='.iconv('windows-1251','utf-8',$chl_salary).'&chdl='.iconv('windows-1251','utf-8',$chdl_salary);
...
$size='700x250';
$img_src_cash_distribution='http://chart.apis.google.com/chart?chof=gif&cht=p&chco=008000&chd=t:'.$chd_cash.'&chs='.$size.'&chl='.iconv('windows-1251','utf-8',$chl_cash).'&chdl='.iconv('windows-1251','utf-8',$chdl_cash);
...
$payments_q=mysqli_query($link,"SELECT id,name FROM base_payment order by name");

$table2_1="";
while ($payments = mysqli_fetch_object($payments_q)) {
    $payments_count=mysqli_result(mysqli_query($link,"SELECT COUNT(id) FROM base_companies
        WHERE id IN(".$companies_string.") AND payment_id='".$payments->id."'"),0,0);
    $payments_percent=$payments_count/$companies_count*100;
    $table2_1.="<tr>
        <td class='left'>".$payments->name."</td><td>".$payments_percent."%</td>
</tr>";
}
...
while ($periods = mysqli_fetch_object($periods_q)) {
    $periods_count=mysqli_result(mysqli_query($link,"SELECT COUNT(id) FROM base_companies
        WHERE id IN(".$companies_string.") AND period_id='".$periods->id."'"),0,0);
...
while ($freq = mysqli_fetch_object($freq_q)) {
    $freq_count=mysqli_result(mysqli_query($link,"SELECT COUNT(id) FROM base_companies WHERE
        id IN(".$companies_string.") AND freq_id='".$freq->id."'"),0,0);
...
while ($currency = mysqli_fetch_object($currency_q)) {
    $currency_count=mysqli_result(mysqli_query($link,"SELECT COUNT(id) FROM base_companies WHERE
        id IN(".$companies_string.") AND currency_id='".$currency->id."'"),0,0);
...
/********************************************************/
    ...
    $q_people_compensations=mysqli_query($link,"select * from base_people_compensations where job_id='$job_id'
            AND company_id IN(".$companies_string.")");
...
$q_archive=mysqli_query($link,"INSERT INTO for_users_corporat_reports values('','$user_id','$url','$date','1','$job_id','$city_id')");
