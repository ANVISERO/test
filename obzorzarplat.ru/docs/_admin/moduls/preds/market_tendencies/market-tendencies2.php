...
$period=$_POST['period'];
  ...
  if($city_id==0){
    $q_period="WHERE id IN (SELECT jtype_id FROM base_job_types
                      WHERE job_id IN (SELECT job_id FROM base_company_jobs bcj
                      WHERE quarter='$period'))";;
    $q_period_job="AND id IN (SELECT job_id FROM base_company_jobs bcj
                      WHERE quarter='$period')";
  }else{
    $q_period="WHERE id IN (SELECT jtype_id FROM base_job_types
                      WHERE job_id IN (SELECT job_id FROM base_company_jobs bcj
...
$q_jtype=mysqli_query($link,"SELECT * from base_jtype ".$q_period." order by name");
?>
...
$q_jobs=mysqli_query($link,"select * from base_jobs where id in(select job_id from base_job_types where jtype_id='$jtype_id') ".$q_period_job." order by name");
