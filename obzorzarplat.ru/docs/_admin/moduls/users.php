...
if($test_col==0){include($folder.'_admin/moduls/users-avtor.php');}
if($test_col<>0){
$kov="'";
...
fputs ( $file, $user_id);
fclose ($file);
echo('<script>self.location.href='.$kov.'/users/'.$kov.'</script>');
}
...
{include($folder.'_admin/moduls/users-avtor.php');}

if(file_exists($folder.'_admin/elements/users/enter/'.$date.'/'.$user_ip.'.txt'))
{include($folder.'_admin/moduls/users-room.php');}
