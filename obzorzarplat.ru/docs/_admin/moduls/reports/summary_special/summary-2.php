...
case "n":
  if($city_id==0){
    ...
    $q_factor="WHERE id IN (SELECT jtype_id FROM base_job_types
                      WHERE job_id IN (SELECT job_id FROM base_company_jobs WHERE city_id='$city_id'))";
    $q_factor_job="id IN (SELECT job_id FROM base_company_jobs WHERE city_id='$city_id')";
  }
  break;
}
}
...
if($lists!=="on"){
    $q_jtype=mysqli_query($link,"SELECT id,name from base_jtype ".$q_jtype_block." order by name");
}
?>
