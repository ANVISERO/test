...
$expiration_date=date('Y')."-".$monthId_now."-01";
$exp_date=strtotime($expiration_date);

if($dateMax<$exp_date){$dateMax=date('Y-m-d',$exp_date);}else{$dateMax=date('Y-m-d',$dateMax);}

list($year,$monthId,$day)=implode('[-,/,.]',$dateMax);

$month=array('1'=>'январь', '01'=>'январь',
...
function table1_3($array) {
  $array=delete_from_array($array,'0');
  $table[0]=number_format(min($array), 0, ',', ' ');
