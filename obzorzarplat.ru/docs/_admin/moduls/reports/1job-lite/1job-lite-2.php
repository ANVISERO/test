...
$reports=mysqli_result(mysqli_query($link,"SELECT reports from for_users_corporat WHERE id='".$_SESSION["user_id"]."'
AND password='".$_SESSION["psw"]."' "),0,0);

//переписываем кол-во отчетов
$reports++;
mysqli_query($link,"update for_users_corporat set reports='$reports' WHERE id='".$_SESSION["user_id"]."'
AND password='".$_SESSION["psw"]."'");
...
$q_archive=mysqli_query($link,"INSERT INTO for_users_corporat_reports values('','$user_id','$url','$date','2','$job_id')");
...
include($folder.'_admin/moduls/reports/1job-lite/1job-lite-html.php');
include($folder.'_admin/moduls/reports/1job-lite/1job-lite-excel.php');
include($folder.'_admin/moduls/reports/1job-lite/1job-lite-2-echo.php');
}
?>