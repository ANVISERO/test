...
session_start ();
		...
		if ( ! $_SESSION['logged_in'] ) {
			...
			if ( isset ( $_COOKIE['cookie_id'] ) ) {//if we have a cookie
				...
				$query =  'SELECT * FROM for_users_corporat WHERE ID = ' . $db->qstr ( $_COOKIE['cookie_id'] );
		...
		if ( $access == FALSE ) {
			header ( "Location: http://obzorzarplat.ru/tests/login/");
		}		
	}
	...
	function get_level_access ( $user_id )
	{
                           $link = mysqli_connect($host,$user,$pass);
                           mysqli_select_db($link,$db);
		$row = mysqli_result(mysqli_query($link,"SELECT levelaccess FROM for_users_corporative WHERE id = '$user_id '"),0,0);
		return $row;
	}
