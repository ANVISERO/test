...
$col_elem=mysqli_num_rows(mysqli_query($link,"select * from for_news where status='1' and lang='1'"));
...
$filter="select * from for_news where status='1' and lang='1' order by date desc limit $start_lim,$col_topage";
$kov="'";
...
<tr>
  ...
  <td valign="top" style="border-bottom:1px dotted #cccccc"><a href="/eng/news/view/?id='.$row["id"].'" style="font-size:11px; font-weight:bold; font-style:normal; text-decoration:none; color:#cc0000">'.$row["zag"].'</a></td>
</tr>
<tr>
...
{
echo('
<a href="/eng/news/view/?id='.$row["id"].'"><img style="border:1px solid #2b2b2b; cursor:hand" width="80" height="80" src="'.$filefolder.$row["id"].'.jpg" alt="Detail. '.$row["zag"].'"></a>');
}
echo('</td>
<td valign="top">
'.$anons.'<br><br>
<input type="button" value="Detail..." class="but" onClick="self.location.href='.$kov.'/eng/news/view/?id='.$row["id"].$kov.'"></td>
</tr>
</table>
');
$marker=-$marker;
