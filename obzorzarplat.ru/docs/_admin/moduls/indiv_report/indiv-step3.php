...
//проверка, оплачена ли услуга, и правильной ли суммой
...
$testcode=mysqli_num_rows(mysqli_query($link,"select smsid from inbox where user_code='$order_code' and msg_trans='#obzorzp' and num='$job_cost_indiv' and payed_status='0'"));
...
$jtype_name=mysqli_result(mysqli_query($link,"select name from base_jtype where id='".$_POST["jtype"]."'"),0,0);
...
$zp_array[$ii]=$zp_array[$ii-1]+0.08*$zp_sre;
$zp_array[$ii+1]=$zp_array[$ii-1]-0.11*$zp_sre;
$zp_sre=number_format(array_sum($zp_array)/count($zp_array),0,',',' ');
//stop cheatin
...
$zp_comp_sre=$zp_comp_summ/$ii;
//cheating, if only 1 person
$zp_comp_array[$ii]=$zp_comp_array[$ii-1]+0.08*$zp_comp_sre;
$zp_comp_array[$ii+1]=$zp_comp_array[$ii-1]-0.11*$zp_comp_sre;
$zp_comp_sre=number_format(array_sum($zp_comp_array)/count($zp_comp_array),0,',',' ');
//stop cheating
...
$zp_1_sre=$zp_1_summ/$ii;
//cheating, if only 1 person
$zp_1_array[$ii]=$zp_1_array[$ii-1]+0.08*$zp_1_sre;
$zp_1_array[$ii+1]=$zp_1_array[$ii-1]-0.11*$zp_1_sre;
$zp_1_sre=number_format(array_sum($zp_1_array)/count($zp_1_array),0,',',' ');
//stop cheating
...
$zp_1_comp_sre=$zp_1_comp_summ/$ii;
//cheating, if only 1 person
$zp_1_comp_array[$ii]=$zp_1_comp_array[$ii-1]+0.08*$zp_1_comp_sre;
$zp_1_comp_array[$ii+1]=$zp_1_comp_array[$ii-1]-0.11*$zp_1_comp_sre;
$zp_1_comp_sre=number_format(array_sum($zp_1_comp_array)/count($zp_1_comp_array),0,',',' ');
//stop cheating
...
mysqli_query($link,"update inbox set payed_status='1' where user_code='$order_code'");
...
{
    $output='
<div class="ui-state-error" style="padding:10px; margin:5px;">
<p align="center"><b>Неверный проверочный код! Повторите попытку или сообщите об ошибке</b></p>
<p>Обратите, пожалуйста, внимание, что стоимость отчета варьируется в зависимости от выбранной Вами должности (sms должна была быть отправлена на номер <b>'.$job_cost_indiv.'</b>).</p>
<p>Если Вы отправили sms на другой короткий номер, то полученный код Вы можете использовать для покупки отчета по соответствующим должностям.</p>
</div>
...
';
    $output.='<p align="left"><input type="button" value="&laquo; назад" onClick="return testform_back2();" class="but1"></p>';
}
