...
function Cell($w,$h=0,$txt='',$border=0,$ln=0,$align='',$fill=0='',$currentx=0) //EDITEI
{
	//Output a cell
	$k=$this->k;
...
//$op='S';//DEBUG
    ...
    //End of coordinate adjustment
		...
		if($link!='') $this->Link($this->x+$dx,$this->y+.5*$h-.5*$this->FontSize,$this->GetStringWidth($txt),$this->FontSize);
	}
	if($s) $this->_out($s);
	$this->lasth=$h;
...
function Write($h,$txt,$currentx=0='') //EDITEI
{
	...
	//Last chunk
	if($i!=$j) $this->Cell($l/1000*$this->FontSize,$h,substr($s,$j),0,0,'',0);
}
...
function Image($file,$x,$y,$w=0,$h=0,$type=''='',$paint=true)
{
