...
<p class="no_print"><input type="submit" value="&laquo; Новый отчет" onclick="self.location.href=\'/work/scoring-entrepreneur/\';" class="submit"></p>
');}
...
$filter="select (official_salary+add_payment+bonus+premium) as salary from base_entrepreneur where job_id='$job_id' and region_id='$region_id' order by official_salary";
...
if(mysqli_num_rows(mysqli_query($link,$filter))==0){
    ...
    $filter="select (official_salary+add_payment+bonus+premium) as salary from base_entrepreneur where job_id='$job_id' and region_id='1' order by official_salary";
    }else{
...
$q_archive=mysqli_query($link,"INSERT INTO zarplata_banks.for_users_corporat_scoring_reports values(0,'$url','$user_id','$job_id','$region_id','$date','$borrower_salary','$score','$proc_90_salary', '1', '$salary_sre')");
...
include($folder.'application/moduls/reports/scoring-entrepreneur/scoring-html.php');
include($folder.'application/moduls/reports/scoring-entrepreneur/scoring-2-echo.php');
}
?>