...
<p class="no_print"><input type="submit" value="&laquo; Новый отчет" onclick="self.location.href=\'/work/scoring-entrepreneur-large/\';" class="submit"></p>
');}
...
//$filter="select (official_salary+add_payment+bonus+premium) as salary from base_entrepreneur_large where job_id='$job_id' and region_id='$region_id' order by official_salary";
$filter="select (official_salary+add_payment+bonus+premium) as salary from base_entrepreneur_large where job_id in (select job_id from base_entrepreneur_service_jobs where service_id = '$job_id') and region_id='$region_id' order by official_salary";
...
if(mysqli_num_rows(mysqli_query($link,$filter))==0){
    ...
    $filter="select (official_salary+add_payment+bonus+premium) as salary from base_entrepreneur_large where job_id in (select job_id from base_entrepreneur_service_jobs where service_id = '$job_id') and region_id='1' order by official_salary";
	//echo $filter;
    }else{
...
$q_archive=mysqli_query($link,"INSERT INTO zarplata_banks.for_users_corporat_scoring_reports values('','$url','$user_id','$job_id','$region_id','$date','$borrower_salary','$score','$proc_90_salary', '2', '$salary_sre')");
...
include($folder.'application/moduls/reports/scoring-entrepreneur-large/scoring-html.php');
include($folder.'application/moduls/reports/scoring-entrepreneur-large/scoring-2-echo.php');
}
?>