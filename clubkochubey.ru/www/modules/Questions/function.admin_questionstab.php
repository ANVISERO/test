...
$questiontypes = array($this->Lang('all_questions')=>'ALL',
...
if( isset($params['applyfilter']) )
  {
    if( isset( $params['input_questiontype'] ) )
      {
	$this->SetPreference(QUESTIONS_FILTER_QUESTIONTYPES,
    ...
    if( isset( $params['input_subject_keywords'] ) )
      {
	$this->SetPreference(QUESTIONS_FILTER_SUBJECT_KEYWORDS,
    ...
    if( isset( $params['input_category'] ) )
      {
	$this->SetPreference(QUESTIONS_FILTER_CATEGORY,
    ...
    if( isset( $params['input_showchildcategories'] ) )
      {
	$this->SetPreference(QUESTIONS_FILTER_SHOWCHILDCATEGORIES,
	...
	is_numeric( $params['input_pagelimit'] ) )
      {
	$this->SetPreference(QUESTIONS_FILTER_PAGELIMIT,
...
$smarty->assign('legend_filter_settings',
...
$smarty->assign('input_showchildcategories',
		$this->CreateInputCheckbox($id,'input_showchildcategories','yes',
					   $showchildcategories));
	       
$smarty->assign('prompt_questiontype', $this->Lang('prompt_filter_questiontype'));
$smarty->assign('input_questiontype',
...
$smarty->assign('input_subject_keywords',
...
$this->smarty->assign('addlink', $this->CreateLink($id, 'admin_addquestion', $returnid, $gCms->variables['admintheme']->DisplayImage('icons/system/newobject.gif', $this->Lang('prompt_addquestion'),'','','systemicon'), $parms, '', false, false, '') .' '. $this->CreateLink($id, 'admin_addquestion', $returnid, $this->Lang('prompt_addquestion'), $parms, '', false, false, 'class="pageoptions"'));
...
if( $pagenumber == 1 )
  ...
  {
    $smarty->assign('prevpage',
		    $this->CreateLink($id,QUESTIONS_ACTION_DEFADMIN,
				      $returnid,'<',
...
$falseimage1 = $gCms->variables['admintheme']->DisplayImage('icons/system/false.gif','not answered','','','systemicon');
$trueimage1 = $gCms->variables['admintheme']->DisplayImage('icons/system/true.gif','answered','','','systemicon');
$falseimage2 = $gCms->variables['admintheme']->DisplayImage('icons/system/false.gif','not approved','','','systemicon');
$trueimage2 = $gCms->variables['admintheme']->DisplayImage('icons/system/true.gif','approved','','','systemicon');
...
while( $dbresult && $row =& $dbresult->FetchRow() )
  {
    ...
    $parms = array('question_id'=>$row['id'],
		   QUESTIONS_PARAM_ACTIVETAB=>QUESTIONS_QUESTIONSTAB);
