...
while( !$dbresult->EOF )
  {
    ...
    $depth = count(split('\.', $dbresult->fields['hierarchy']));
    $onerow->id = $dbresult->fields['id'];
    $onerow->name = str_repeat('&nbsp;', $depth-1).$this->CreateLink($id,'editcategory',$returnid,
    ...
    $onerow->editlink = $this->CreateLink($id, 'editcategory', $returnid, 
					  $gCms->variables['admintheme']->DisplayImage('icons/system/edit.gif', $this->Lang('edit'),'','','systemicon'), array('catid'=>$dbresult->fields['id']));
    $onerow->deletelink = $this->CreateLink($id, 'deletecategory', $returnid, $gCms->variables['admintheme']->DisplayImage('icons/system/delete.gif', $this->Lang('delete'),'','','systemicon'), 
					    array('catid'=>$dbresult->fields['id'],
						  QUESTIONS_PARAM_ACTIVETAB=>QUESTIONS_CATEGORIESTAB), 
