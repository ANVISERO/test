...
if( !isset($params['messagelist']) )
  {
    $params['error'] = 1;
    $params['message'] = $this->Lang('error_nomessagesselected');
...
if( !isset( $params['jobname'] ) || $params['jobname'] == '' )
  {
    $params['error'] = 1;
    $params['message'] = $this->Lang('error_nojobname');
    $this->Redirect($id,'create_edit_job',$returnid,$params);
    return;
  }
$jobid='';
...
if( $dbresult && $dbresult->RecordCount() )
  {
    $params['error'] = 1;
    $params['message'] = $this->Lang('error_jobnameexists');
    $this->Redirect($id,'create_edit_job',$returnid,$params);
    return;
  }
...
if( !$dbresult )
  {
    $params['error'] = 1;
    $params['message'] = $this->Lang('error_dberror').' '.$db->ErrorMsg();
    $this->Redirect($id,'create_edit_job',$returnid,$params);
    return;
  }
