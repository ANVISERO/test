...
if(!$this->CheckPermission('LinkMgr: modify templates') ) exit;
...
if(isset($params['submit'])) {
	...
	if($template_text != '' && $template_name != '') {
		...
		$entry_id = $db->GenID(cms_db_prefix()."module_linkmgr_templates_seq");

		$query = 'INSERT INTO ' . cms_db_prefix() . 'module_linkmgr_templates (entry_id, name, content) 
					VALUES (?, ?, ?)';
...
$smarty->assign('startform', $this->CreateFormStart($id, 'admin_addtemplate', $returnid));
