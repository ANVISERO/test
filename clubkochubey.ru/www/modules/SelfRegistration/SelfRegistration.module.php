...
#-------------------------------------------------------------------------
class SelfRegistration extends CMSModule
{
  /*---------------------------------------------------------
...
//       case 'verifyuser':
  ...
  /*---------------------------------------------------------
   ...
   ---------------------------------------------------------*/
  function _CreateFrontendUser( $tmpid, $group_id, $username, $password, $do_md5 = true )
  {
    global $gCms;
    $feusers =& $this->GetModuleInstance('FrontEndUsers');
    if( !$feusers )
      {
	return $this->Lang('error_nofeusersmodule');
      }

    // todo, figure out when users should expire
    $timeperiod = $feusers->GetPreference('expireage_months',520);
	...
	if( !$feusers->AssignUserToGroup( $uid, $gid ) )
	  {
	    return $this->Lang('warning_couldnotaddgroup');
	  }
      }
...
//     echo $this->ProcessTemplateFromDatabase('selfreg_finalmessage_template');
  ...
  function CreateTempUser( $gid, $username, $password, $code )
  {
    $db = $this->GetDb();
    ...
    $q = "INSERT INTO ".cms_db_prefix()."module_selfreg_users
          VALUES (?,?,?,?,?,?)"; 
    $dbresult = $db->Execute( $q, array( $id, $gid, $username, md5($password), $code,
