...
class FrontEndUsers extends CMSModule 
{
  /**
...
<!-- Logout form template -->
 ...
 /*---------------------------------------------------------
  ...
  function SetParameters()
  {
    ...
    $this->RegisterRoute('/[fF]uo\/reset\/(?P<uid>[0-9]+)$/',
 ...
 function _HasSufficientPermissions( $perm = '' )
 {
  ...
  function _DoSetChangeSettingsTemplate( $id, &$params, $returnid )
  {
    ...
    $this->myRedirectToTab($id, 'changesettings_template' );
  }
  ...
  function _DoSetForgotPWTemplate( $id, &$params, $returnid )
  {
    ...
    $this->myRedirectToTab($id, 'forgotpassword_template' );
  }
  ...
  function _DoSetLoginTemplate( $id, &$params, $returnid )
  {
    ...
    $this->myRedirectToTab($id, 'logintemplate' );
  }
  ...
  function _DoSetLogoutTemplate( $id, &$params, $returnid )
  {
    ...
    $this->myRedirectToTab($id, 'logouttemplate', $params);
  }
  ...
  function _DoUserAction( $id, &$params, $returnid )
  {
    if( !isset($params['form']))
      {
	// Do an attempt to login via cookie
	$this->_AttemptLoginWithCookie();
	if( !$this->LoggedIn() ) 
	  {
	    $params['form'] = 'login';
	  }
  ...
  // This function is used for edit, and add user
  function _DisplayAdminAddUserStep2Page( $id, &$params, $returnid )
  {
    global $gCms;
      ...
      if( isset( $params['user_id'] ) )
	{
      ...
      $onerow->hidden = RRUtils::myCreateInputHidden( $id, 'hidden_'.$prop['name'],
					     implode(";",array($prop['name'],$defn['type'],
							       $defn['length'],$prop['required'],
							       $defn['maxlength'],$propval)));
	...
	case 1: // checkbox
	  $onerow->control = RRUtils::myCreateInputCheckbox( $id, 'input_'.$prop['name'], 1, $val );
	  break;
    ...
    if( isset($params['input_username']) )
      {
	$this->smarty->assign('edittext',$this->Lang('editing_user'));
	$this->smarty->assign('username',trim($params['input_username']));
      }
    $this->smarty->assign('hidden', 
			  RRUtils::myCreateInputHidden( $id, 'step1_params', $params ));
    ...
    $this->smarty->assign('cancel',$this->CreateInputSubmit($id,'cancel',
							    $this->Lang('cancel')));
    $this->smarty->assign('back',$this->CreateInputSubmit($id,'back',
							    $this->Lang('back')));
    if( isset( $params['user_id'] ) )
      {
    $this->smarty->assign('title', $this->Lang('edituser'));
	$this->smarty->assign('hidden2', 
			      $this->CreateInputHidden($id,'user_id',$params['user_id']));
...
function myRedirectToTab( $id, $tab, $params = '', $ignore_returnto = false )
  {
    $parms = array();
    if( is_array( $params ) )
      {
    ...
    $this->myRedirect( $id, 'defaultadmin', '', $parms, $ignore_returnto );
  }
...
EOT;
  ...
  function ManageImageUpload($id, $fldprefix, $fldname, $uid, $image_width = '')
  {
    ...
    $destDir = $gCms->config['uploads_path'].DIRECTORY_SEPARATOR;
    $destDir .= $this->GetPreference('image_destination_path', 'feusers');
  ...
  function _AttemptLoginWithCookie()
  {
	...
	$res = $this->Login( $arr['u'], $arr['p'], '', true, true );
      }
  }
...
function AddGroupPropertyRelation( $grpid, $propname, $sortkey, $lostun, $val )
 ...
 function AddPropertyDefn( $name, $prompt, $type, $length, $maxlength = 0, $attribs = '' )
  {
    $this->_load();
    return $this->usermanip->AddPropertyDefn( $name, $prompt, $type, $length, $maxlength, $attribs );
  }
 ...
 function GetMemberGroups($userid) 
  ...
  {
    $this->_load();
    return $this->usermanip->IsValidEmailAddress( $email, $uid );
  }
  ...
  {
    $this->_load();
    return $this->usermanip->IsValidUsername( $username );
  }
  ...
  {
    $this->_load();
    return $this->usermanip->LoggedIn();
  }
  ...
  {
    $this->_load();
    return $this->usermanip->SetPropertyDefn( $name, $newname, $prompt, $type, $length, $maxlength, $attribs );
  }
  ...
  function GetContentBlockInput($blockName,$value,$params,$adding = false)
  {
	...
	return $this->CreateInputDropdown('',$blockName,$groups,-1,$value);
	break;
      }
    return FALSE;
  }
