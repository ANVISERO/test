...
function check_group_complete( &$grp )
{
  ...
  if(!isset($grp['name']) || empty($grp['name']))
    {
      return false;
    }
  
  // passed
  if( !isset($grp['properties']) || !is_array($grp['properties']) ||
      (count($grp['properties']) == 0) )
    {
      return false;
    }
  ...
  foreach( $grp['properties'] as $oneprop )
    {
      if( !check_property_complete( $oneprop ) )
	{
	  return false;
	}
    }
...
function check_property_complete( &$prop )
{
  ...
  if(!isset($prop['prompt']) || empty($prop['prompt']))
    {
      return FALSE;
    }
  if(!isset($prop['sortorder']) || empty($prop['sortorder']))
    {
      return FALSE;
    }
  if(!isset($prop['status']) || empty($prop['status']))
    {
      return FALSE;
    }
  if(!isset($prop['lostunflag']) || empty($prop['lostunflag']))
    {
      return FALSE;
    }
  ...
  if(isset($prop['maxlength']))
    {
      if(empty($prop['maxlength']))
	{
	  return FALSE;
	}
      if(intval($prop['maxlength']) == 0)
	{
	  return FALSE;
	}
    }
  ...
  if( $type == 4 || $type == 5 )
	...
	{
	  if( !isset($oneop['name']) || !isset($oneop['text']) )
	    {
	      return FALSE;
	    }
	}
...
if( isset( $params['submit'] ) )
  {
	...
	$parms['error'] = $this->Lang('error_missing_upload');
	$this->Redirect($id,'defaultadmin',$returnid, $parms);
	return;
      }
    $thefile =& $_FILES[$id.'xmlfile'];
    if( $thefile['type'] != 'text/xml' ||
	$thefile['size'] == 0 ||
	$thefile['error'] != 0 )
      {
	$parms = array();
	$parms['active_tab'] = 'groups';
	$parms['error'] = $this->Lang('error_problem_upload');
	$this->Redirect($id,'defaultadmin',$returnid, $parms);
	return;
      }
	...
	$parms = array();
	$parms['active_tab'] = 'groups';
	$parms['error'] = $this->Lang('error_bad_xml').' 1';
	$this->Redirect($id,'defaultadmin',$returnid, $parms);
	return;
      }
	...
	switch( $tag )
	  ...
	  case 'feu_group':
	      ...
	      if( !check_group_complete($grp_info) )
		{
		  ...
		  $parms = array();
		  $parms['active_tab'] = 'groups';
		  $parms['error'] = $this->Lang('error_bad_xml').' 2';
		  $this->Redirect($id,'defaultadmin',$returnid, $parms);
		  return;
		}
	  ...
	  case 'grp_description':
	      ...
	      if( isset($grp_info[$fld]) )
		{
		  ...
		  $parms = array();
		  $parms['active_tab'] = 'groups';
		  $parms['error'] = $this->Lang('error_bad_xml').' 3';
		  $this->Redirect($id,'defaultadmin',$returnid, $parms);
		  return;
		}
	      $grp_info[$fld] = $value;
	  ...
	  case 'property':
	      ...
	      if( !check_property_complete($oneproperty) )
		{
		  ...
		  $parms = array();
		  $parms['active_tab'] = 'groups';
		  $parms['error'] = $this->Lang('error_bad_xml').' 4';
		  $this->Redirect($id,'defaultadmin',$returnid, $parms);
		  return;
		}
	      if( !isset($grp_info['properties']) )
		{
		  $grp_info['properties'] = array();
		}
	  ...
	  case 'prop_lostunflag':
	      ...
	      if( isset( $oneproperty[$fld] ) )
		{
		  ...
		  $parms = array();
		  $parms['active_tab'] = 'groups';
		  $parms['error'] = $this->Lang('error_bad_xml').' 5';
		  $this->Redirect($id,'defaultadmin',$returnid, $parms);
		  return;
		}
    ...
    if( isset($params['input_newname']) &&
	$params['input_newname'] != '' )
      {
	$grp_info['name'] = $params['input_newname'];
      }
	...
	$parms = array();
	$parms['active_tab'] = 'groups';
	$parms['error'] = $this->Lang('error_groupexists');
	$this->Redirect($id,'defaultadmin',$returnid, $parms);
	return;
      }

    if( !isset($grp_info['properties']) ||
	!is_array($grp_info['properties']) ||
	(count($grp_info['properties']) == 0) )
      {
	// no properties?
	$parms = array();
	$parms['active_tab'] = 'groups';
	$parms['error'] = $this->Lang('error_properties');
	$this->Redirect($id,'defaultadmin',$returnid, $parms);
	return;
      }
	...
	$parms = array();
	$parms['active_tab'] = 'groups';
	$parms['error'] = $this->Lang('error_dup_properties');
	$this->Redirect($id,'defaultadmin',$returnid, $parms);
	return;
      }
    ...
    foreach( $grp_info['properties'] as $oneprop )
      {
	$res = $this->AddPropertyDefn($oneprop['name'],
				      $oneprop['prompt'],
				      $oneprop['type'],
				      $oneprop['lengtn'],
				      $oneprop['maxlength']);
	if( is_array( $res ) && $res[0] === FALSE )
	  {
	    ...
	    $parms = array();
	    $parms['active_tab'] = 'groups';
	    $parms['error'] = $this->Lang('error_cantaddprop');
	    $this->Redirect($id,'defaultadmin',$returnid, $parms);
	    return;
	  }
    ...
    $res = $this->AddGroup($grp_info['name'],
			   $grp_info['description']);
    if( is_array( $res ) && $res[0] === FALSE )
      {
	$parms = array();
	$parms['active_tab'] = 'groups';
	$parms['error'] = $this->Lang('error_cantaddgroup');
	$this->Redirect($id,'defaultadmin',$returnid, $parms);
	return;
      }
    $grpid = $res[1];
	...
	// add the options
	$res = $this->AddGroupPropertyRelation( $grpid, 
						$oneprop['name'],
						$oneprop['sortorder'], 
						$oneprop['lostunflag'], 
						'' );
	...
	$parms = array();
	$parms['active_tab'] = 'groups';
	$parms['error'] = $this->Lang('error_cantaddgroupreln');
	$this->Redirect($id,'defaultadmin',$returnid, $parms);
	return;
      }
...
$smarty->assign('startform',
		$this->CreateFormStart($id,'admin_importgroup',$returnid,
...
$smarty->assign('input_filename',
		$this->CreateFileUploadInput($id,'xmlfile','',40));
$smarty->assign('prompt_newname',$this->Lang('prompt_newgroupname'));
$smarty->assign('input_newname',
		$this->CreateInputText($id,'input_newname'));
