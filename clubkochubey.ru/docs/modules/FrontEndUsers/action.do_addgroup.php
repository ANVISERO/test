...
if( isset($params['serialized']) )
  {
    $params['input_name'] = unserialize($params['input_name']);
    $params['input_required'] = unserialize($params['input_required']);
  }

$groupname = '';
if( isset( $params['input_groupname'] ) )
  {
    $groupname = trim($params['input_groupname']);
  }
$groupdesc = '';
if( isset( $params['input_groupdesc'] ) )
  {
...
for( $i = 0; $i < count($params['input_name']); $i++ )
  {
    ...
    if( $val != 0 && $propname != '')
      {
	$this->AddGroupPropertyRelation( $gid, $propname, $i, $lostun, $val );
      }
  }
...
$this->myRedirectToTab($id, 'groups' ); 
// EOF
?>