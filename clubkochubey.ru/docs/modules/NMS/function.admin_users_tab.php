...
$query= "SELECT userid FROM ".NMS_USERS_TABLE;
$dbresult= $db->Execute($query);
$total_recs = 0;
if( $dbresult )
  {
    $total_recs= $dbresult->RecordCount();
  }
...
if (isset ($params['filter'])) // the button
  {
    ...
    if (isset ($params['users_per_page']))
      {
	$this->SetPreference('users_per_page', min((int)$params['users_per_page'],10000));
      } 
  }
...
$num_per_batch= max(1,$this->GetPreference('users_per_page', 25));
...
$query= "SELECT A.userid,uniqueid,email,username,disabled,confirmed,count(B.listid) as lists FROM ".NMS_USERS_TABLE." A LEFT OUTER JOIN ".NMS_LISTUSER_TABLE." B ON A.userid = B.userid";
$where= false;
if ($nms_user_filter != '')
  {
    if ($where == false)
    ...
    $query .= " A.email REGEXP \"$nms_user_filter\"";
  }
  if ($nms_username_filter != '')
  {
    ...
    $query .= " A.username REGEXP \"$nms_username_filter\"";
  }
if ($nms_list_filter != '')
  {
	...
	$query .= " B.listid IN ($nms_list_filter)";
      }
  }
...
if ($total_recs > $num_per_batch)
  {
    $numpages= intval($total_recs / $num_per_batch);
    if ($numpages * $num_per_batch < $total_recs)
      {
	$numpages++;
      }
    if ($page > 1)
