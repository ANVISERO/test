...
//Avoid problem name with CGExtensions
function forum_html_entity_decode_utf8( $string )
{
	static $trans_tbl;
	//replace numeric entities
	$string = preg_replace('~&#x0*([0-9a-f]+);~ei', 'forum_code2utf(hexdec("\\1"))', $string);
	$string = preg_replace('~&#0*([0-9]+);~e', 'forum_code2utf(\\1)', $string);
