...
#
#------------------------------------------------------------------------
#$Id: action.default.php 99 2010-04-24 20:43:16Z alby $
...
if(!empty($params['forum']))
{
	...
	$arr_url['forum'] = $forum;
	$where .= (!empty($forum)) ? "id IN ({$forum})" : '';
	$subnodes = $this->TreeManager->customQuery('', $where, array(), true, 'lft', 'id', 1);
}
else
{
	$root = $this->TreeManager->getRoot();
	$subnodes = $this->TreeManager->getSubNodes($root, -1, 'lft', true, 'id');
}
...
$smarty->assign('breadcrumbs', false);
