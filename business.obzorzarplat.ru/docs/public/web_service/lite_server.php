...
class SalaryService{
    ...
    public function __destruct(){
        //connecting to DB
        /*
        $this->connect($this->folder);
        $user_corporat_id=mysqli_result(mysqli_query($link,"select id from for_users_corporat
                                        where login='".$_SERVER['PHP_AUTH_USER']."' AND password='".md5($_SERVER['PHP_AUTH_PW'])."'"),0,0);
        mysqli_query($link,"insert into for_temp_users_corporat (user_corporat_id,reports,money,last_accessed)
                            VALUES('".$user_corporat_id."','".$this->reports."','".$this->money."','".date('Y-m-d, H-i-s')."')");
        echo($this->reports);
         * 
         */
    }
   ...
   function getSalary($params){
        ...
        //functions
        include($this->folder.'_admin/moduls/reports/funcs.php');

        $job=$params["job"];
        $city=$params["city"];

        $query=mysqli_query($link,"select * from archive_people where job_id='".$job."' AND region_id='".$city."' AND official_salary>0 AND date like '2008%'");
...
mysqli_query($link,"insert into for_temp_users_corporat (user_corporat_id,last_accessed)
                            VALUES('".$user_corporat_id."','".date('Y-m-d, H-i-s')."')");
...
/*
$test=new SalaryService();
$job='1374';
$city='1';

$params=array('job'=>$job, 'city'=>$city);
$output=$test->getSalary($params);
...
echo("<p>".$test->getJobName($job)."</p>");
echo("<p>".$test->getCityName($city)."</p>");
*/
$server=new SoapServer("http://localhost/business/web_service/lite.wsdl", array('encoding'=>'windows-1251'));
