...
function percentile($data,$percentile){
  ...
  if($data==0){$result=0;} else {
    ...
    for($i = 0; $i < $count; $i++){
      $curr_salary = $data[$i];
      $curr_stat = $i/($count - 1);
      $table_item = array('salary'=>$curr_salary, 'stat'=>$curr_stat);
      $table[] = $table_item;
    }
    ...
    foreach($table as $k=>$table_item){
      if($k >= $count - 1){
        break;
      }
      if(($p >= $table_item['stat']) && ($p <= $table[$k+1]['stat'])){
        $table_item1 = $table_item;
        $table_item2 =  $table[$k+1];
        break;
      }
    }


    $result = $table_item1['salary'] + ($table_item2['salary'] - $table_item1['salary']) * (($p - $table_item1['stat']) / ($table_item2['stat'] - $table_item1['stat']));
...
if(date('n',$dateMax)==1 || date('n',$dateMax)==2 || date('n',$dateMax)==3)	    {$quarter=3;}
elseif(date('n',$dateMax)==4 || date('n',$dateMax)==5 || date('n',$dateMax)==6) {$quarter=2;}
elseif(date('n',$dateMax)==7 || date('n',$dateMax)==8 || date('n',$dateMax)==9) {$quarter=3;}
else {$quarter=3;}
...
//$dateUpdate=$quarter.' квартал '.date('Y');
...
function salary_indexes($array) {
  ...
  if(empty ($array)){
    ...
    ///print_r($array);
    /*
	$table[0]=number_format(percentile($array, 10)/0.87, 0, ',', ' ');
    $table[1]=number_format(percentile($array, 25)/0.87, 0, ',', ' ');
    $table[2]=number_format(average($array)/0.87, 0, ',', ' ');
    $table[3]=number_format(percentile($array, 50)/0.87, 0, ',', ' ');
    $table[4]=number_format(percentile($array, 75)/0.87, 0, ',', ' ');
    $table[5]=number_format(percentile($array, 90)/0.87, 0, ',', ' ');
	*/
...
function count_people_new($array) {
   if(count($array)){
  	$interval = array_unique($array);
  		foreach($array as $cid){
    		foreach($interval as $int){
...
function table_type1($string,$comp_people,$people_sum,$sum,$compensation_id,$politics_people,$link){
...
function table_type2($part,$string,$comp_people,$people_sum,$sum,$compensation_id,$politics_people,$link) {
$number=1;

foreach($string as $m){
$query=mysqli_query($link,"SELECT * FROM base_compensations where type_id='$m'");
$out1[$m]=
'<table border="1" class="business">
<tr class="X1_0">
  <th width="50%"></th>
  <th width="20%"></th>
  <th width="10%">Доля сотрудников*</th>
