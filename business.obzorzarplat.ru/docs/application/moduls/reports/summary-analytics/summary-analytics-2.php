...
if($city_id==0 || empty ($job) || empty ($factor)){ ?>
    <p>Кажется, Вы выбрали не все факторы для формирования отчета</p>
    <p align="right"><input type="button" value="Новый отчет &raquo;" class="submit" onclick="self.location.href='/work/summary-analytics/';"></p>
<?php
    }else{
    //Название города
if($city_id==0){
...
case "personal":
  ...
  if($city_id==0){
  $query[$job_id]="SELECT id,company_id,official_salary,
      add_payment,bonus,premium,compensation,payment_id,period_id,date
      FROM base_people where job_id='$job_id'
  AND company_id in(select id from base_companies
  where personal_id='$factor_id')";
  }else{
  $query[$job_id]="SELECT id,company_id,official_salary,
      add_payment,bonus,premium,compensation,payment_id,period_id,date FROM base_people where job_id='$job_id'
  AND region_id='$city_id'
  AND company_id in(select id from base_companies
  where personal_id='$factor_id')";
  }
...
$q_archive=mysqli_query($link,"INSERT INTO for_users_corporat_reports values(0,'$user_id','$url','$date','6','null','$city_id')");
...
include($folder.'application/moduls/reports/summary-analytics/summary-analytics-html.php');
include($folder.'application/moduls/reports/summary-analytics/summary-analytics-excel.php');
include($folder.'application/moduls/reports/summary-analytics/summary-analytics-2-echo.php');
}
?>